...
const convertToJSON = (data: string): string => {
  // Split JSONL by newlines and filter out empty lines
...
const JsonlToJsonConverter: React.FC = () => {
    ...
    (acceptedFiles: File[]) => {
      if (acceptedFiles.length > 1) {
        toast({
          ...
          description: "Please upload only one JSONL file.",
        });
        return;
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e: ProgressEvent<FileReader>) => {
        if (e.target?.result) {
          ...
          if (offset >= fileSize) {
            // Validate that the file contains valid JSONL (at least one valid JSON object per line)
            const lines = result.split("\n").filter((line) => line.trim());
            if (lines.length === 0) {
              reject(new Error(`Empty or invalid JSONL file: ${file.name}`));
              return;
            }
            try {
              lines.forEach((line) => JSON.parse(line.trim())); // Validate each line
              resolve(result);
            } catch (err) {
              reject(
                new Error(
                  `Invalid JSONL in ${file.name}: ${(err as Error).message}`
                )
              );
            }
          } else {
  ...
  const processInput = async () => {
    let jsonlText = "";
    if (pastedInput.trim()) {
      jsonlText = pastedInput;
    } else if (file) {
      try {
        jsonlText = await readFileInChunks(file);
      } catch (error) {
        setOutputContent("");
        toast({
          variant: "destructive",
      ...
      toast({
        ...
        description: "Please either upload a file or paste JSONL text.",
      });
      return;
    }
    ...
    try {
      ...
      const jsonOutput = convertToJSON(jsonlText);
      setOutputContent(jsonOutput);
      setProgress(100);
      toast({
        title: "Conversion Successful",
        description: "JSONL has been converted to JSON successfully.",
      });
    } catch (error) {
      setOutputContent("");
      toast({
  ...
  return (
    ...
    >
      <Card className="sm:mb-2 bg-gray-950 text-white">
        <CardHeader>
          <CardTitle>JSONL to JSON Converter</CardTitle>
          <CardDescription className="text-gray-100">
            Convert your JSON Lines files to structured JSON format quickly and
            efficiently.
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <Dropzone
            ...
            accept={{ "text/plain": [".jsonl", ".ndjson"] }}
          >
          ...
          {/* Pasted JSONL input */}
          <div className="mt-6">
            <Label className="text-gray-300">Or Paste JSONL Here</Label>
            <textarea
              value={pastedInput}
              onChange={handlePastedInputChange}
              placeholder="Paste your JSONL text here..."
              rows={6}
...
export default JsonlToJsonConverter;
