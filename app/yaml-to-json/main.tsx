...
const convertToJSON = (data: string): string => {
  try {
    const parsedYaml = YAML.parse(data, { prettyErrors: true });
...
const YamlToJsonConverter: React.FC = () => {
      ...
      reader.onload = (e: ProgressEvent<FileReader>) => {
        if (e.target?.result) {
          ...
          if (offset >= fileSize) {
            ...
            try {
              YAML.parse(result, { prettyErrors: true });
              resolve(result);
            } catch (err) {
              reject(
                new Error(
  ...
  const processInput = async () => {
    ...
    try {
      ...
      const jsonOutput = convertToJSON(yamlText);
      ...
      <Card className="sm:mb-2 bg-gray-950 text-white">
        <CardHeader>
          ...
          <CardDescription className="text-gray-100">
            Convert your YAML files to structured JSON format quickly and
            efficiently.
          </CardDescription>
        </CardHeader>
        <CardContent>
